...
contract('House', accounts => {
    ...
    describe('during session zero', () => {
        ...
        it("disallows users from liquidating house credits when it isn't a profit distribution period", async () => {
            ...
            let sessionCredits = userCredits[0].toFixed()
            await utils.assertFail(
                house.liquidateCredits(currentSession, sessionCredits)
            )
        })
    ...
    describe('during session one', () => {
        ...
        it('disallows users from liquidating credits', async () => {
            ...
            currentSession = currentSession.toNumber()
            await utils.assertFail(
                house.liquidateCredits(currentSession, creditsToLiquidate, {
                    from: nonFounder
                })
            )
        })
        ...
        it(
            ...
            async () => {
                ...
                houseBalance = houseBalance.toFixed(0)
                console.log(
                    'House balance after depositing to newBettingProvider',
                    houseBalance
                )
                ...
                houseBalance = houseBalance.toFixed(0)
                console.log(
                    'House balance after depositing to slotsChannelManager',
                    houseBalance
                )
                ...
                houseBalance = houseBalance.toFixed(0)
                console.log(
                    'House balance after depositing to bettingProvider',
                    houseBalance
                )
